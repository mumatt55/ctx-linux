...
pkgListUpdateRhel8=("java-17-openjdk")
...
function getSysInfo()
{
    ...
    # MUST follow Ubuntu, because Pardus utilizes the same file(lsb-release) as Ubuntu.
       ...
       if [[ "$?" -ne "0" || -z "${osVersion}" ]]; then           
        ...
        #UOS20 1070
        if [[ "$osPlatform" == "uos" ]]; then
            osPlatform="debian"  # UOS 基于 Debian 10，复用 debian 逻辑
        fi
        if [[ "$?" -ne "0" || -z "${osVersion}" ]]; then           
            myPrint "$infoVersion"
            exit 1
...
function osPlatformCheck()
{
    ...
    if [[ "$osPlatform" == "red" ]]; then
        if [[ "${osVersion}" == "7.9" || "${osVersion}" == "7.8" || "${osVersion}" == "8.1" || "${osVersion}" == "8.2" || "${osVersion}" == "8.3" || "${osVersion}" == "8.6" || "${osVersion}" == "8.4" || "${osVersion}" == "8.8" || "${osVersion}" == "8.10" ]]; then
        ...
        if [[ "${osVersion}" == "8.6" || "${osVersion}" == "8.8" || "${osVersion}" == "8.10" ]]; then
    ...
    #UOS20 1070
    elif [[ "${osPlatform}" == "debian" || "${osPlatform}" == "uos" ]]; then
    ...
    #UOS20 1070
    elif [[ "${osPlatform}" == "uos" ]]; then
        if [[ "${osVersion}" == "20" ]]; then
            info="$(get_str CTXINSTALL_PLATFORM_INFO_PRECHECK  2  'UOS' ${osVersion})"
            myPrint "$info"
        else
            info="$(get_str CTXINSTALL_VERSION_INFO_PRECHECK 2 'UOS' ${osVersion})"
            myPrint "$info"
            preCheckStatus="Warning"
        fi
...
function repositoryCheck()
{
    ...
    #elif [[ "${osPlatform}" == "ubuntu" || "${osPlatform}" == "pardus" || "${osPlatform}" == "debian" ]]; then
    #UOS20 1070
    elif [[ "${osPlatform}" == "ubuntu" || "${osPlatform}" == "uos" || "${osPlatform}" == "pardus" || "${osPlatform}" == "debian" ]]; then
    ...
    while true; do 
        ...
        if [[ "$version" == "8" ]]; then
            ...
            if [[ "$joinDomainWay" == "sssd" ]]; then
                ...
                # cache_credential should be set to False
                `sed -i '/.*cache_credentials =.*$/c\cache_credentials = False' "$sssdFile"`
